...
type ArticleWithBlocks = Prisma.ArticleGetPayload<{
  include: {
    blocks: {
      include: {
        ratings: true
...
type BlockWithRelations = ArticleWithBlocks["blocks"][number]
  ...
  try {
    ...
    if (id) {
      ...
      const transformedArticle = {
        ...
        keyIngredients: article.blocks
          .filter((block) => block.ingredientsList && block.ingredientsList.length > 0)
          .flatMap((block) => block.ingredientsList.map((ingredient) => ingredient.name)),
    ...
    const transformedArticles = articles.map((article) => {
      ...
      return {
        ...
        description: descriptionBlock?.content?.replace(/<\/?[^>]+(>|$)/g, "").substring(0, 150) + "..." || "",
