...
const BlockTypeIndicator = ({ type }: { type: string }) => (
  <div className="w-2 h-2 bg-blue-500 rounded-full" title={type} />
)

const BlockConverter = ({ block, onConvert }: { block: Block; onConvert: (type: Block["type"]) => void }) => (
  <select
    value={block.type}
    onChange={(e) => onConvert(e.target.value as Block["type"])}
  ...
  >
    ...
    <option value="productRating">Product Rating</option>
    <option value="prosCons">Pros & Cons</option>
    <option value="ingredientsSection">Ingredients</option>
...
const ImageUploader = ({
  ...
  <div className="space-y-2">
    ...
    />
    {initialImage && <Image src={initialImage || "/placeholder.svg"} alt="Preview" className="max-w-xs h-auto rounded" />}
  </div>
)
    ...
    {items.map((item) => (
      <button
        ...
        className={`px-3 py-1 text-sm rounded whitespace-nowrap ${
          activeId === item.id ? "bg-blue-500 text-white" : "bg-gray-100 hover:bg-gray-200"
        }`}
      >
        {item.title}
      </button>
...
const ProductRatingSection = ({
  ...
  <div className="border rounded-lg p-4 space-y-4">
    <div className="flex items-center gap-4">
      {imageUrl && (
        <Image src={imageUrl || "/placeholder.svg"} alt={productName} className="w-20 h-20 object-contain rounded" />
      )}
      <div>
    ...
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
      <div>
        ...
        <div>{ingredientsRating}/5</div>
      </div>
      <div>
        <div className="font-medium">Value</div>"use client"
...
export default BlockEditor
        ...
        <div>{valueRating}/5</div>
      </div>
      <div>
        <div className="font-medium">Manufacturer</div>
        <div>{manufacturerRating}/5</div>
      </div>
      <div>
        <div className="font-medium">Safety</div>
        <div>{safetyRating}/5</div>
      </div>
    </div>
...
const IngredientsSection = ({
  ...
  if (isEditing) {
    return (
      <div className="space-y-4">
        ...
        <div>
          <Label>Ingredients (JSON format)</Label>
          <Textarea
            value={JSON.stringify(ingredients, null, 2)}
            onChange={(e) => {
              try {
            ...
            placeholder="Enter ingredients as JSON..."
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-2xl font-bold mb-4">{productName} Ingredients</h3>
        ...
        {ingredients.map((ingredient) => (
          <div key={ingredient.id} className="border rounded-lg p-4">
            <div className="flex items-start gap-4">
              <div className="w-16 h-16 bg-gray-100 rounded-lg flex items-center justify-center font-bold text-xl">
                {ingredient.number}
              </div>
              <div className="flex-1">
                ...
                <div className="text-sm text-gray-500">
                  <strong>Study ({ingredient.studyYear}):</strong> {ingredient.studySource} -{" "}
...
export function BlockEditor({ blocks, onChange }: BlockEditorProps) {
  ...
  // Helper to convert array of objects to array of strings for BlockEditor
  function extractContent(arr: any[] | undefined): string[] {
    if (!arr) return []
                      ...
                      <div className="space-y-4 border rounded-md p-4">
                        ...
                        <div className="pt-4 border-t">
                          ...
                          <div className="border rounded-md p-4 bg-white">
                            <ProsConsSection
                              pros={extractContent(block.pros)}
                              cons={extractContent(block.cons)}
                              ingredients={extractContent(block.ingredients)}
                            />
                          </div>
                        </div>
                      </div>
