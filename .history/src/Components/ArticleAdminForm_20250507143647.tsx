  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    if (name.includes(".")) {
      ...
      setFormData({
        ...formData,
        [parent]: {
          ...formData[parent as keyof ArticleData],
          [child]: value,
        },
      })
    } else {
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="w-full">
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 mb-6 border-b">
                {["basic", "benefits", "ratings", "highlights", "ingredients", "details", "faqs", "reviews"].map(
                  (tab) => (
                    <button
                      key={tab}
                      type="button"
              ...
              <div className={`space-y-4 ${activeTab !== "faqs" ? "hidden" : ""}`}>
                ...
                {formData.faqs.map((faq, index) => (
                  <div key={index} className="border p-4 rounded-md relative">
                    ...
                    <div className="grid grid-cols-1 gap-4">
                      <div>
                        <label htmlFor={`faqs.${index}.question`} className="block text-sm font-medium text-gray-700 mb-1">
                          Question
                        </label>
                        <input
                          type="text"
                      ...
                      <div>
                        <label htmlFor={`faqs.${index}.answer`} className="block text-sm font-medium text-gray-700 mb-1">
                          Answer
                        </label>
                        <textarea
                          value={faq.answer}
