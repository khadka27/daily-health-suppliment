...
const ArticleWrapper: React.FC<ArticleWrapperProps> = ({ slug }) => {
      ...
      <div className="space-y-6">
        {articleData.blocks.map((block) => (
          <div key={block.id} className="mb-6">
            {block.type === "heading" && (
              <h2
                className={`font-bold text-blue-600 ${block.level === 1 ? "text-2xl" : block.level === 2 ? "text-xl" : "text-lg"} mb-4`}
              >
                {block.content}
              </h2>
            )}
            ...
            {block.type === "productReview" && (
              <div className="bg-gray-50 p-6 rounded-lg shadow-sm my-6">
                ...
                {block.ratings && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    {block.ratings.ingredients && (
                      <div>
                        <span className="font-medium">Ingredients:</span> {block.ratings.ingredients}/5
                      </div>
                    )}
                    {block.ratings.value && (
                      <div>
                        <span className="font-medium">Value:</span> {block.ratings.value}/5
                      </div>
                    )}
                    {block.ratings.manufacturer && (
                      <div>
                        <span className="font-medium">Manufacturer:</span> {block.ratings.manufacturer}/5
                      </div>
                    )}
                    {block.ratings.safety && (
                      <div>
                        <span className="font-medium">Safety:</span> {block.ratings.safety}/5
                      </div>
                    )}
                    {block.ratings.effectiveness && (
                      <div>
                        <span className="font-medium">Effectiveness:</span> {block.ratings.effectiveness}/5
                      </div>
                    )}
                  </div>
                )}
                ...
                {/* Pros and Cons */}
                {((block.pros && block.pros.length > 0) || (block.cons && block.cons.length > 0)) && (
