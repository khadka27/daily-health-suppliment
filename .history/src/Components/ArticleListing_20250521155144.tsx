...
export default function ArticleListing() {
  ...
  useEffect(() => {
    const fetchArticles = async () => {
      ...
      try {
        ...
        console.log("API URL:", `/api/article?${queryParams.toString()}`);
  ...
  function FormattedDate({ dateString }: { dateString: string }) {
    ...
    useEffect(() => {
      // Formata a data só no cliente para evitar mismatch SSR/CSR
              ...
              <p className="text-xs sm:text-sm mb-2 sm:mb-3">
                ...
                {result.updatedAt && (
                  <span className="ml-2 text-gray-500">
                    • Updated: <FormattedDate dateString={result.updatedAt} />
                  </span>
                )}
              </p>
