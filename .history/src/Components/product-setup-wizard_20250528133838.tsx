...
export function ProductSetupWizard({ onComplete, onCancel }: ProductSetupWizardProps) {
  ...
  // Helper functions for correct type structure
  const makePros = (arr: string[]) => arr.map(content => ({ id: uuidv4(), content }))
  const makeCons = (arr: string[]) => arr.map(content => ({ id: uuidv4(), content }))
  const makeIngredients = (arr: string[]) => arr.map(content => ({ id: uuidv4(), name: content }))
    ...
    headings.forEach((heading, index) => {
      ...
      if (heading === "How Does " + name + " Rate According to Our Experts?") {
        section.push({
          ...
          highlights: [
            { id: uuidv4(), content: "Made with clinically-tested, powerfully effective ingredients.", order: 1, blockId: "" },
            { id: uuidv4(), content: "Manufactured in the USA", order: 2, blockId: "" },
            { id: uuidv4(), content: "Made in GMP-certified facilities to ensure product safety and quality", order: 3, blockId: "" },
          ],
