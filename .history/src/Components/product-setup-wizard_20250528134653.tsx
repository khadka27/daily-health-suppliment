...
export function ProductSetupWizard({ onComplete, onCancel }: ProductSetupWizardProps) {
  ...
  const makePros = (arr: string[], blockId: string) => arr.map((content, i) => ({ id: uuidv4(), content, order: i, blockId }))
  const makeCons = (arr: string[], blockId: string) => arr.map((content, i) => ({ id: uuidv4(), content, order: i, blockId }))
  const makeIngredients = (arr: string[], blockId: string) => arr.map((content, i) => ({ id: uuidv4(), content, order: i, blockId }))
  const makeHighlights = (arr: string[], blockId: string) => arr.map((content, i) => ({ id: uuidv4(), content, order: i, blockId }))
