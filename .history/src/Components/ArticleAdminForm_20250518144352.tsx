...
// Update the handleSubmit function in your ArticleAdminForm component
const handleSubmit = async (e: React.FormEvent) => {
  e.preventDefault();
  
  // Make sure all required fields are present
  const requiredFields = ['title', 'description', 'overview'];
  const missingFields = requiredFields.filter(field => !formData[field as keyof Article]);
  ...
  numericFields.forEach(field => {
    ...
    if (typeof value === 'string') {
      (processedFormData as any)[field] = parseFloat(value) || 0;
    }
  });
  
  // Process customer reviews to ensure ratings are numbers
  if (Array.isArray(processedFormData.customerReviews)) {
    processedFormData.customerReviews = processedFormData.customerReviews.map(review => ({
      ...review,
      rating: typeof review.rating === 'string' ? parseFloat(review.rating) || 0 : review.rating
    }));
  }
  
  // Dispatch the action to save the article
